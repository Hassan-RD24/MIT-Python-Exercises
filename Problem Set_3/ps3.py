...
def get_word_score(word, n):
    ...
    letter_points = sum(SCRABBLE_LETTER_VALUES.get(letter, 0) for letter in word)
    length_bonus = max(7 * word_len - 3 * (n - word_len), 1)

    return letter_points * length_bonus
...
def deal_hand(n):
    ...
    At least one letter in the hand should be the wildcard '*'.
    ceil(n/3) letters in the hand should be VOWELS (note,
    ceil(n/3) means the smallest integer not less than n/3), and the
    wildcard takes up one of those vowel slots.
    ...
    if n > 0:
        # One of the "vowel" slots is replaced by the wildcard.
    ...
    for i in range(int(math.ceil(n / 3)), n):
...
def is_valid_word(word, hand, word_list):
    ...
    Wildcard letters ('*') may stand in for any vowel; this function
    checks whether any substitution of the wildcard with a vowel
    produces a word in word_list.
    ...
    if WILDCARD in word:
        idx = word.find(WILDCARD)
        is_real_word = False
        for vowel in VOWELS:
            candidate = word[:idx] + vowel + word[idx + 1:]
...
def substitute_hand(hand, letter):
    ...
    possible_letters = [c for c in letter_pool if c not in hand and c != letter]
    if not possible_letters:
        # fallback in the unlikely case every letter in the pool is already in hand
        possible_letters = [c for c in letter_pool if c != letter]
